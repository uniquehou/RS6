...
if __name__ == '__main__':
    model = ALS()
    X = pd.read_csv('ratings_small.csv')[['userId', 'movieId', 'rating']]
    model.fit(np.array(X), k=3, max_iter=2)

    user_ids = range(1,13)
    predictions = model.predict(user_ids, n_items=2)
    for user_id, prediction in zip(user_ids, predictions):
        _pre = [f"item_id:{item_id} score:{score:.2f}" for item_id, score in prediction]
        print(f"User id: {user_id}: recommedation: {_pre}")
