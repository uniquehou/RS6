...
tag_items = dict()
user_items = dict()
test_user_tags, test_tag_items, test_user_items = dict(), dict(), dict()
...
def recommend(uid, n):
    ...
    for tid, nt in test_user_tags[uid].items():
        sum_nt = sum(test_user_items[uid].values())
        if tag_items.get(tid):
            for iid, ni in tag_items[tid].items():
                sum_ni = sum(tag_items[tid])
                if iid not in recommend_item.keys():
                    recommend_item[iid] = (nt/sum_nt) * (ni/sum_ni)
                else:
                    recommend_item[iid] += (nt/sum_nt) * (ni/sum_ni)
